...
train_size = df.index.values[-1] - int(df.index.values[-1] * TRAIN_TEST_SPLIT_PERCENT)
train_df = df.loc[:train_size]
trade_df = df.loc[train_size + 1 :]
trade_df = trade_df.pivot(index="Date", columns="Ticker", values="Close").rename_axis(
    None, axis=1
)
...
df = pd.read_parquet(
    results_dir / "all-model-eval-results.parquet", engine="fastparquet"
)
df


best_model = (
    df.groupby(["model"]).last().sort_values("holdings", ascending=False).iloc[0].name
)
best_model


best_model_df = df[df["model"] == best_model].reset_index(drop=True)
first_row = {"model": best_model, "holdings": 100_000, "reward": 0}
...
best_model_df.columns = [
    col + " Buy/Sell" if ".NS" in col else col for col in best_model_df.columns
]
best_model_df
...
daily_return_df = pd.Series(
    daily_return_df["daily_return"], index=daily_return_df.index
)
daily_return_df
